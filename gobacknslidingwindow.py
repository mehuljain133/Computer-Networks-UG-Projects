...
class GoBackNProtocol:
    ...
    def simulate(self):
        ...
        while window_start < len(self.frames):
            ...
            # If ACK is received for all frames, slide the window
            if all(ack):
                print(f"Sender: All frames from {window_start + 1} to {window_end} acknowledged successfully.\n")
                window_start = window_end  # Slide the window
            else:
                print(f"Sender: Error detected in frames. Retransmitting from frame {window_start + 1}...\n")
            
            time.sleep(1)  # Simulate delay in the network
    
    # To simulate transmission of multiple frames
    def simulate_multiple_frames(self):
        print("Starting Go-Back-N Protocol simulation...\n")
        self.simulate()

# Example usage of the Go-Back-N Protocol with noisy channel simulation
if __name__ == "__main__":
    ...
    # Create an instance of the Go-Back-N Protocol with window size 3 and 10% error
    protocol = GoBackNProtocol(frames, window_size=3, error_probability=0.1)
